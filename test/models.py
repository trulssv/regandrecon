...
class RegistrationCNN3D(nn.Module):
    """3D counterpart of RegistrationCNN: takes a 3D template and target volume and outputs a velocity
    field that can be used to deform the template to match the target."""

    def __init__(self, input_channels=2, hidden_channels=32, output_channels=3):
        super(RegistrationCNN3D, self).__init__()
        ...
        self.conv3 = nn.Conv3d(hidden_channels, output_channels, kernel_size=3, padding=1)

    def forward(self, template, target):
        x = torch.stack((template, target), dim=1)  # (B, 2, D, H, W)
        x = F.relu(self.conv1(x))
        x = F.relu(self.conv2(x))
        velocity_field = self.conv3(x)  # (B, 3, D, H, W)

        velocity_field = velocity_field.permute(0, 2, 3, 4, 1)  # (B, D, H, W, 3)

        return velocity_field


class DoubleConv3D(nn.Module):
    """Two 3x3x3 convolutions, each followed by group normalization and a ReLU activation."""
...
class Up3D(nn.Module):
    """Upscaling block: trilinear upsample, concatenate with the skip connection, then a double convolution.
...
class UNet3D(nn.Module):
    """A UNet architecture for estimating a velocity field from a 3D template and target volume.

    3D counterpart of UNet: same encoder-decoder structure with skip connections, but using Conv3d/MaxPool3d
    so it operates on volumetric (D, H, W) data instead of 2D images.
    """

    def __init__(self, in_channels=2, out_channels=3, base_channels=32, depth=4):
        super().__init__()
        self.depth = depth
        ...
        self.downs = nn.ModuleList([Down3D(channels[i], channels[i + 1]) for i in range(depth)])
        self.ups = nn.ModuleList(
            [Up3D(channels[i + 1], channels[i], channels[i]) for i in reversed(range(depth))]
        )
    ...
    def forward(self, template, target):
        ...
        velocity_field = self.outc(x)  # (B, 3, D, H, W)
        velocity_field = velocity_field.permute(0, 2, 3, 4, 1)  # (B, D, H, W, 3)
